...
export class MainUIComponent implements OnInit {
  ...
  change_Menu(tab : string){
      ...
      document.getElementById('refinedPanel').style.display = 'none';
      document.getElementById('problemdiagramPanel').style.display = 'none';
      document.getElementById('droolsrulesPanel').style.display = 'none';
      document.getElementById(tab).style.background = '#166dac'
      document.getElementById('refined').style.background = '#62a0cc'
      document.getElementById('problemdiagram').style.background = '#62a0cc'
  ...
  generateRefinedRequirements(){
    this.generateService.refineRequirements(this.requirementTexts.split('\n').join('//'),this.ontologyFilePath).subscribe(result=>{
      ...
      document.getElementById("refined").style.display = 'block';
      this.change_Menu("refined")
    })
  }

  generateProblemDiagram(){
      this.pfService.getProblemDiagram(this.refindeRequirements.split('\n').join('//'),this.ontologyFilePath).subscribe(result=>{
  ...
  generateDroolsRules(){
    this.generateService.transformToDrools(this.refindeRequirements.split('\n').join('//'),this.ontologyFilePath).subscribe(result=>{
      console.log(result)
